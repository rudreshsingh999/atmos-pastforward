...
while True:
	...
	for x in range(10):
		if(time[x][0]==1000):
			break
		board.Servos.write(gates[time[x][1]-1],100)   #opening of gate
		flag=0
		s1 = time.time();
        while((time.time()-s1)<(time[x][0]+0.1)):
        	if(board.digitalRead(11)==1):
        		flag=1
        board.Servos.write(gates[time[x][1]-1],0)  #closing of gate
        if(flag==0):
        	board.Servos.write(gates[time[x][1]-1],100)
        	while(board.digitalRead(11)!=1):
        		pass
